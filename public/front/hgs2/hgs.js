...
var hgs = {
	...
	km = {
		...
		setInvoiceType: function(that) {
			var thisID = parseInt(that.data("id")),
				placeholder = "";
			$(".invoice-type").removeClass("btn-fill"), that.addClass("btn-fill"), 0 === thisID ? placeholder = "Şasi Numarası" : 1 === thisID && (placeholder = "Vergi No"), $("#invoice-no").attr("placeholder", placeholder)
		},
		paymentConfirm: function() {
			app.clearToastr();
...
var traffic = {
	...
	setTaxpayerType: function(that) {
		var changeTitle, changePlaceholder, normalQuery = $(".tp_normal_flow_query"),
			...
			text = that.html(),
			queryName = "";
		selectedTaxpayerType.find("b").html(text), normalQueryTitle.find("b").html(typeTitle), that.parent().find("li").removeClass("btn-fill"), that.addClass("btn-fill"), $("input[name=taxpayer_type]").val(type), "real" === type ? (changeTitle = "Sasi No", changePlaceholder = "Örn: 01234567890", queryName = "T.C. Kimlik numarasını") : "juridical" === type && (changeTitle = "VERGÄ° KÄ°MLÄ°K NO", changePlaceholder = "Örn: 0123456789", queryName = "Vergi Kimlik numarasını"), changeField.find(".tp_normal_flow_table_title").html(changeTitle), changeField.find(".tp_normal_flow_form_title input").attr("placeholder", changePlaceholder), changeField.find(".tp_normal_flow_form_title input").val(""), $(".tp-heading-query-name").html(queryName), normalQuery.show()
	},
